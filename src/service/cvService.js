...
const CvService = {
  evaluateDeeplabPredictions (predictions, cv) {
    let width = 224
    let height = 224
    ...
    return { error: false, diameter: trunkDiameter }
    // return trunkDiameter;
  },
  greyscaleMask: function (canvas, cv) {
    let closingFactor = 20
    let openingFactor = 20
    let allAccuracies = []
    let goneWrong = 0
    ...
    if (trunkDiameter > -99999) {
      return { error: false, diameter: trunkDiameter }
    } else {
      return { error: true, diameter: trunkDiameter }
    }

    /* cv.imshow('output-cardrectangle', cardRect);
    cv.imshow('output-trunkrectangle', trunkRect);
    cv.imshow('output-cardopened', openedCard);
    cv.imshow('output-trunkopened', openedTrunk);
    console.log(absolutAbweichungen);
    console.log(prozentualeAbweichungen);
    console.log(realDiameters); */
  }
}
